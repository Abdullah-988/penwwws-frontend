...
export default function GoogleOAuthBtn({ redirectUrl }: Props) {
  const URL = `https://accounts.google.com/o/oauth2/auth?redirect_uri=http://localhost:3000/auth/session/callback&response_type=token&client_id=${process.env.GOOGLE_CLIENT_ID}&scope=openid%20email%20profile&state=${redirectUrl}`;
  return (
